  ...
  description: INodeTypeDescription = {
    ...
    properties: [
      ...
      },
      {
        ...
        default: '',
        displayOptions: { show: { resource: ['blog'], operation: ['create'] } },
        description: 'Fallback language, e.g. German or de. A language in the prompt takes priority. Leave empty to use the website language.',
      },
      {
        displayName: 'Schedule At',
        name: 'scheduleAt',
      ...
      },
      {
        ...
        displayOptions: { show: { resource: ['blog'], operation: ['batchCreate'] } },
        description:
          'JSON array of up to 25 blog creation items. Each item must have a `prompt` (min 10 chars). Optional: `schedule_at` (ISO 8601), `language` (name or code).',
      },
      // Blog Publish fields
      {
        displayName: 'Website ID',
    ...
    for (let i = 0; i < items.length; i++) {
      try {
        ...
        if (resource === 'blog' && operation === 'create') {
          ...
          const language = (this.getNodeParameter('blogLanguage', i, '') as string).trim();
